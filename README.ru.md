...
Опция `--no-minimize` выключает эту минимизацию.
...
CSS-файле заменяет его на имя, основанное на хеш-сумме. В конфиге `.borschik`
...
Опция `--no-freeze` выключает заморозку.
