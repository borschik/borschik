...
exports.process = function(opts) {
    ...
    if (stat.isFile()) {
        processFile(input, process.cwd(), result);

    } else if (stat.isDirectory()) {
        readDir(input, basePath, result);
    }

    return U.writeFile(output, opts.minimize? JSON.stringify(result) : JSON.stringify(result, null, 4));
};
