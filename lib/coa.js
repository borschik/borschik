...
module.exports = require('coa').Cmd()
    ...
    .opt()
        ...
        .def(true)
        .val(function(v) {
            return util.stringToBoolean(v, true);
        })
        .end()
    .opt()
        ...
        .end()
    .opt()
        .name('cache').title('Use file cache')
        .long('cache')
        .def(false)
        .val(function(v) {
            return util.stringToBoolean(v, false);
        })
        .end()
    .opt()
        .name('tmpDir').title('Use file cache')
        .long('tmp-dir')
        .val(function(path) {
            return path ? PATH.resolve(path) : PATH.join(process.cwd(), '.borschik_tmp');
        })
        .end()
    .opt()
    ...
    .cmd()
        ...
        .act(function(opts) {
            ...
            return util.writeFile(opts.output, opts.minimize? JSON.stringify(result) : JSON.stringify(result, null, 4));
        })
        .end()
    .act(function(opts) {
        var tech = opts.tech;
