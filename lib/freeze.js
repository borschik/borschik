...
var realpathSync = exports.realpathSync = function(path, base) {
    ...
    for (var i = 0; i < folders.length; ) {
        ...
        if (_followSymlinks && isSymLink(PATH.join(prefix, name))) {
            var realRelativePath = FS.readlinkSync(PATH.join(prefix, name));
            var realAbsolutePath = PATH.resolve(prefix, realRelativePath);
            var linkParts = realRelativePath.split(PATH.sep);
            // if symlink is a directory
            if (FS.statSync(realAbsolutePath).isDirectory()) {
                // adds one more "go to parent"
                linkParts.unshift('..');
            }
