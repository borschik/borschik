...
var freeze = exports.freeze = function(filePath, content, webp) {
    ...
    if (_freezeDir) {
        ...
        if (_freezeDir === ':base64:' || _freezeDir === ':encodeURIComponent:') {
            ...
            var excluded = config.webp.excludePath ? !!_freezeDir.match(config.webp.excludePath) : false;
            
            if ( (webp && !excluded) || !CWEBP) {
...
var loadConfig = exports.loadConfig = function(path) {
    ...
    if (FS.existsSync(config_path)) {
        ...
        if (webp) {
            ...
            config.webp.excludePath = _config.webp.exclude_path ? new RegExp(_config.webp.exclude_path) : false;
        }
 ...
 */
 var convertToWebp = exports.convertToWebp = function (inputFilePath, outputFilePath, callback){
    ...
    EXEC(CWEBP + params.join(" "), function (error, stdout, stderr) {
        if (error !== null) {
            console.error(error);
        }
        if (callback) {
