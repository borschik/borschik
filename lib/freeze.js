...
var freeze = exports.freeze = function(filePath, content) {
    ...
    if (_freezeDir) {
        ...
        // Special syntax for resource inlining
        var inliningSyntax = typeof _freezeDir === 'string' && _freezeDir.match(/:(\w+):/);
        // Type of resource inlining
        var inliningType = inliningSyntax ? inliningSyntax[1] : null;
        // Size limit for resource inlining
        var inliningLimit;

        // Support for literal object rule for inlining size limit (https://github.com/bem/borschik/issues/82)
        if (typeof _freezeDir === 'object') {
            inliningType = _freezeDir.inliningType;
            inliningLimit = _freezeDir.inliningLimit;
            // Path for freezing if limit is exceeded
            _freezeDir = _freezeDir.freezePath;
        }

        var fileSize = inliningLimit && FS.statSync(filePath).size;
        var skipInlining = inliningLimit && (fileSize > inliningLimit);

        if ((inliningType === 'base64' || inliningType === 'encodeURIComponent' || inliningType === 'encodeURI') &&
                !skipInlining) {
            ...
            if (inliningType === 'base64') {
                ...
                // data:image/svg+xml,....
                var encodeFunc = inliningType === 'encodeURI' ? encodeURI : encodeURIComponent;
    ...
    if (FS.existsSync(config_path)) {
        ...
        for (var freezeConfigWildcard in freezePaths) {
            ...
            if (!config.freezeWildcards[realpath]) {
                var freezeRule = freezePaths[freezeConfigWildcard];
                // Support for inlining size limit (https://github.com/bem/borschik/issues/82)
                var freezeToPath = typeof freezeRule === 'object' ? freezeRule.freezePath : freezeRule;

                // :base64: and :encodeURIComponent: are special syntax for resource inlining
                if (freezeToPath && freezeToPath !== ':base64:' && freezeToPath !== ':encodeURIComponent:' && freezeToPath !== ':encodeURI:') {
                    ...
                    if (typeof freezeRule === 'object') {
                        freezeRule.freezePath = freezeToPath;
                    } else {
                        freezeRule = freezeToPath;
                    }
                }
                config.freezeWildcards[freezeRealPath] = freezeRule;
            }
        }
