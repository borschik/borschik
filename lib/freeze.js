...
var freeze = exports.freeze = function(filePath, content) {
    ...
    if (_freezeDir) {
        ...
        if (_freezeDir === ':base64:') {
            ...
            return 'data:' + contentType + ';base64,' + new Buffer(content).toString('base64');

        } else if (_freezeDir === ':encodeURIComponent:') {
            var fileExtension = PATH.extname(filePath);
            var contentType = contentTypes[fileExtension];
            if (!contentType) {
                throw new Error('Freeze error. Unknown Content-type for ' + fileExtension);
            ...
            // encode file as base64
            return 'data:' + contentType + ';' + encodeURIComponent(new Buffer(content).toString('utf8'));
