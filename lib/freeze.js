...
var freeze = exports.freeze = function(filePath, content) {
    ...
    if (_freezeDir) {
        ...
        if (_freezeDir === ':base64:' || _freezeDir === ':encodeURIComponent:') {
            ...
            var nestingLevel = freezeNestingLevel(filePath);
...
/**
 * Get freeze nesting level for specific file path.
 *
 * @param {String} filePath File path to use.
 * @returns {Number} Freeze dir.
 */
var freezeNestingLevel = function(filePath) {
    filePath = PATH.normalize(filePath);
    ...
    while (matched = filePath.match(rePrefix)) {
        prefix += matched[0];
        freezeLevel = configFreezeNestingLevel(prefix) || freezeLevel;
...
var loadConfig = exports.loadConfig = function(path) {
    ...
    if (FS.existsSync(config_path)) {
        ...
        if (!config.freezeNestingLevel[realpath]) {
            config.freezeNestingLevel[realpath] = _freezeNestingLevel;
        }
