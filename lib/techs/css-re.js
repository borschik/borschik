    ...
    File: INHERIT(base.File, {
        ...
        processInclude: function(path, content) {
            ...
            for(var i = 0; i < parsed.length; i++) {
                ...
                url = p.resolve(dir, url);
                if (item.type === "import") {
                    var file = p.relative(this.root, url);
                    parsed[i] = '/* ' + file + ' begin */\n' +
                        this.processInclude(url, this.parseInclude(fs.readFileSync(url))) +
                        '\n/* ' + file + ' end */\n';
                } else if (item.type === "url") {
                    parsed[i] = "url(" + p.relative(this.root, url) + ")";
                }
            }
...
function makeParsed(items, content) {
    ...
    if (lastInd < content.length) result.push(new Buffer(content.substring(lastInd), "utf-8"));
