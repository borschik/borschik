...
class File extends BaseFile {
    ...
    async processInclude(path, content) {
        ...
        const promises = parsed.reduce((acc, item) => {
            ...
            acc.push(this.child(item.type, item.url)
                .then(childFile => childFile.process(path))
                .then(childContent => {
                    if (item.type === 'include') {
                        const comments = this.tech.opts.comments;
                        return (comments ? '/* ' + item.url + ' begin */\n' : '') +
                            childContent +
                            (comments ? '\n/* ' + item.url + ' end */\n' : '');
                    }
        ...
        return Promise.all(promises).then(parsed => parsed.join(''));
    }
