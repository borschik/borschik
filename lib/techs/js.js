...
exports.Tech = base.Tech.inherit({
    process: function() {
        ...
        return vow.all([
            ...
            sourceMap ? this.write(this.defaultSourceMapPath(), sourceMap) : vow.resolve()
        ]);
    },
    ...
    defaultSourceMapPath: function() {
        return (this.opts.output ? this.opts.output.path : '') + '.map';
    },
    ...
    buildSourceMapURLComment: function(sourceMap) {
        ...
        if (typeof techOpts.sourceMapURL === 'string') {
            url = techOpts.sourceMapURL;
        } else if (techOpts.sourceMap === 'inline') {
            url = 'data:application/json;base64,' + Buffer.from(sourceMap).toString('base64');
        } else {
            url = path.basename(this.defaultSourceMapPath());
        }
    ...
    File: exports.File = base.File.inherit({
        ...
        processInclude: function(baseFile, content) {
            ...
            var serializer = new NodesSerializer(parsed, {
                ...
                sourceMapSourcesPrefix: techOpts.sourceMapSourcesPrefix,
