...
var NodesSerializer = inherit({
     ...
     * @param {String} [outputSourceMapFilename='']
     */
    ...
    serialize: function() {
        ...
        var baseDir = options.outputSourceMapRoot || path.dirname(options.outputSourceMapFilename);
        ...
        this.nodes.forEach(function addToConcater(node) {
            if (node instanceof nodes.IncludeNode) {
                ...
                concater.concat(path.relative(baseDir, node.path), node, mappings);
            }
