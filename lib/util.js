...
function _getTech(tech, use_cwd) {
    ...
    if (/^[\/.]/.test(tech)) {
        return require(PATH.resolve(process.cwd(), tech));
    }

    // load build-in tech
    try {
        var pathToTech = PATH.resolve(__dirname, '../lib/techs/' + tech);
        return require(pathToTech);
    } catch(e) {}
    ...
    // try to loaed external tech from npm
